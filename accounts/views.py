...
# more complex sanitization for xss (brooa-based data types, can handle strings, dicts, and lists recursively,)
...
def sanitize_input(post_data):
    return {key: escape(value) if isinstance(value, str) else value for key, value in post_data.items()}
...
def register(request):
    ...
    if was_ratelimited:
        messages.error(request, "You have exceeded the maximum number of registration attempts. Please try again later.")
        return redirect("login")
    if request.method == "POST":
        sanitized_data = sanitize_input(request.POST)
        form = StudentAddForm(sanitized_data)
